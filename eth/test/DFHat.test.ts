...
describe('DarkForestHat', function () {
  ...
  it('should only allow owner to buy skin', async function () {
    const planet = await world.contract.planets(SPAWN_PLANET_1.id);
    expect(planet.hatLevel.toNumber()).to.be.equal(0);

    await expect(
      world.user2Core.buySkin(SPAWN_PLANET_1.id, {
        value: '1000000000000000000',
      })
    ).to.be.revertedWith('Only owner account can perform that operation on planet.');
  });

  it('should not buy skin with insufficient value', async function () {
    const planet = await world.contract.planets(SPAWN_PLANET_1.id);
    expect(planet.hatLevel.toNumber()).to.be.equal(0);
    ...
    await expect(
      world.user1Core.buySkin(SPAWN_PLANET_1.id, {
