...
class Repeater {
  ...
  loadAttacks() {
    const attacksJSON = localStorage.getItem(`repeatAttacks-${this.account}-${this.configHash}`); // @ts-ignore
  ...
  addAttack() {
    ...
    attack.pcRemain =
      attack.pcTrigger <= attack.pcRemain ? parseInt(attack.pcTrigger / 2) : attack.pcRemain; // Filter out existing attacks from this source, then add new attack at top
